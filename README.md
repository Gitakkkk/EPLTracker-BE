...
- [x] 회원가입 / 로그인 (JWT + 카카오/네이버/구글 소셜 로그인)
- [x] 경기 목록 조회 (football-data.org 시드 스크립트)
- [x] 픽 제출 (경기 시작 전까지만 가능 — 시간 기반 검증 로직)
- [x] 경기 결과 입력 → 적중 여부 자동 계산 + UserStat 갱신 (트랜잭션 처리)
- [x] 랭킹 조회 (전체 누적 / 라운드별)
...
## 다음 액션 후보
- [x] 데이터 소스 결정 → football-data.org API 시드 스크립트
- [x] 레포 구조 설계 + 초기 스키마(ERD) 설계
- [x] NestJS 프로젝트 초기 세팅
- [ ] 더미 유저 1만 명 + 픽 10만 건 생성 스크립트 (단계 2 진입)
- [ ] 랭킹 쿼리 EXPLAIN ANALYZE → 인덱스 최적화
- [ ] Redis 캐싱 (라운드 랭킹)